...
module.exports = function (movieId) {
  return pull(
    once(movieId),
    asyncMap((movieId, cb) => {
      db('cast_crew').where('movie_id', movieId).select().asCallback(cb)
    }),
    flatten(),
    map((row) => row.person_id),
    collect((err, personIds) => {
      console.log(personIds)
      filmographies(personIds)
    })
  )
}

function filmographies (personIds) {
  let currentMovieid 
  ...
  pull(
    ...
    filter((res) => containsCreative(concat(res.cast, res.crew), personIds)),
    map((res) => {
      rating(currentMovieid)
      return res
    }),
...
function containsCreative (credits, personIds) {
  return some(credits, (credit) => {
    if (isMainCrew(personIds, credit)) {
      console.log('isMainCrew', credit)
    }
    if (isMainActor(personIds, credit)) {
      console.log('isMainActor', credit)
    }
